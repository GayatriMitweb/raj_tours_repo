			...
			<div class="row mg_bt_10">
				<div class="col-xs-12">
				  	<select name="entry_id" id="entry_id" multiple>
				  		<?php 
				  		$sq_ticket_entries = mysql_query("select * from ticket_master_entries where ticket_id='$ticket_id' and status='Cancel'");
				  		while($row_entry = mysql_fetch_assoc($sq_ticket_entries)){
				  			?>
	...
	<div class="col-md-6 col-sm-12 col-xs-12">
		...
		<table class="table table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				$sq_ticket_refund = mysql_query($query);
				while($row_ticket_refund = mysql_fetch_assoc($sq_ticket_refund)){

					$count++;

					$total_refund = $total_refund+$row_ticket_refund['refund_amount'];

					$sq_ticket_info = mysql_fetch_assoc(mysql_query("select * from ticket_master where ticket_id='$row_ticket_refund[ticket_id]'"));

					$traveler_name = "";

					$sq_refund_entries = mysql_query("select * from ticket_refund_entries where refund_id='$row_ticket_refund[refund_id]'");
			...
			<tfoot>
				...
				$sq_refund_can_amount = mysql_fetch_assoc(mysql_query("select sum(refund_amount)as sum from ticket_refund_master where ticket_id='$sq_ticket_info[ticket_id]' and clearance_status='Cancelled'"));
				?>
				<tr class="active">
					...
					<th class="text-right success" colspan="2">Total : <?= number_format(($total_refund - $sq_refund_pen_amount['sum'] - $sq_refund_can_amount['sum']),2) ?></th>
				</tr>
			</tfoot>
		</table>
		</div>
