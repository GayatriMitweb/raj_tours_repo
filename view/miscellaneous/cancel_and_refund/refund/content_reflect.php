...
$sale_amount = $sq_visa_info['misc_total_cost'];
			...
			<div class="row mg_bt_10">				
				<div class="col-xs-12">
				  	<select name="entry_id" id="entry_id" multiple>
				  		<?php 
				  		$sq_visa_entries = mysql_query("select * from miscellaneous_master_entries where misc_id='$misc_id' and status='Cancel'");
	...
	<div class="col-md-7 col-sm-12 col-xs-12">
		...
		<table class="table table-hover table-bordered" id="tbl_refund_list" style="margin: 0 !important;">
			...
			<tbody>
				...
				$total_refund = 0;
				$query = "select * from miscellaneous_refund_master where misc_id='$misc_id' and refund_amount!='0'";
				$count = 0;
				$bg;
				...
				while($row_visa_refund = mysql_fetch_assoc($sq_visa_refund)){
					...
					$sq_visa_info = mysql_fetch_assoc(mysql_query("select * from miscellaneous_master where misc_id='$row_visa_refund[misc_id]'"));
					 $date = $sq_visa_info['created_at'];
			         $yr = explode("-", $date);
			         $year =$yr[0];

					$traveler_name = "";
					$sq_refund_entries = mysql_query("select * from miscellaneous_refund_entries where refund_id='$row_visa_refund[refund_id]'");
					...
					$date1 = $row_visa_refund['refund_date'];
					$yr1 = explode("-", $date1);
					$year1 =$yr1[0];
					...
					$v_voucher_no = get_misc_booking_refund_id($row_visa_refund['refund_id'],$year1);
              ...
              $('#vi_confirm_box').vi_confirm_box({
                ...
                callback: function(data1){
                    if(data1=="yes"){
                        ...
                        $.ajax({
                          type:'post',
                          url: base_url+'controller/miscellaneous/refund/refund_save.php',
                          data:{ misc_id : misc_id, refund_amount : refund_amount, refund_date : refund_date, refund_mode : refund_mode, bank_name : bank_name, transaction_id : transaction_id, bank_id : bank_id, entry_id_arr : entry_id_arr },
